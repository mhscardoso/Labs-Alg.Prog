/*
	Esta sera a implementacao multiplayer do jogo
*/
/*
	Nivel 1 - 4 x 3
	Nivel 2 - 5 x 4
...
int main(void) {
	...
	printf("Mas, antes, quantos jogadores vao participar? ");
	scanf("%i", &q_jogadores);

	printf("Perfeito, serao %d jogadores\nDigam seus nomes:\n", q_jogadores);
	char jogadores[q_jogadores][35];
	int tentativas[q_jogadores];
	for (int i = 0; i < q_jogadores; i++) {
	...
	printf("Agora, escolha um nivel que sera para todos:\n");
	...
	for (int i = 0; i < q_jogadores; i++) {
		...
		conta_tempo("Prepare-se: ", 5);

		clear();
		
		cria_baralho(nivel, mat);

		tentativas[i] = jogo(nivel, mat);
		...
		conta_tempo("Esperando: ", 7);

		clear();
	}

	printf("O vencedor foi %s, que descobriu o baralho em %i tentativas.\n", jogadores[menor], tentativas[menor]);
